...
class _AdminDashboardState extends State<AdminDashboard> {
      ...
      builder: (context, constraints) {
        ...
        return Scaffold(
          ...
          appBar: (isMobile || isTablet) ? AppBar(
            ...
            title: Text(context.translate('admin_panel'), style: const TextStyle(color: Color(0xFF1A1A1A), fontWeight: FontWeight.w900)),
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Expanded(child: ListView(padding: const EdgeInsets.symmetric(horizontal: 12), children: [
          _buildSidebarItem(Icons.dashboard_rounded, context.translate('home'), 0), _buildSidebarItem(Icons.map_rounded, context.translate('track'), 1), _buildSidebarItem(Icons.analytics_rounded, context.translate('analytics'), 2), _buildSidebarItem(Icons.chat_bubble_rounded, context.translate('complaints'), 3), _buildSidebarItem(Icons.people_outline_rounded, context.translate('user_mgmt'), 4), _buildSidebarItem(Icons.storage_rounded, context.translate('data_mgmt'), 5), _buildSidebarItem(Icons.settings_suggest_rounded, context.translate('settings'), 6),
        ])),
        ...
        Padding(padding: const EdgeInsets.only(bottom: 24), child: _buildSidebarItem(Icons.logout_rounded, context.translate('logout'), -1, isLogout: true)),
      ]),
    );
  }
  ...
  Widget _buildStatGrid(bool isMobile, bool isTablet) {
    return GridView.count(
      ...
      children: [
        _buildModernStatCard(context.translate('active_fleet'), "$_activeTrucks", Icons.local_shipping_rounded, const Color(0xFF43A047)),
        _buildModernStatCard(context.translate('issues'), "$_pendingComplaints", Icons.warning_amber_rounded, const Color(0xFFE53935), badge: _pendingComplaints > 0 ? "$_pendingComplaints" : null),
        _buildModernStatCard(context.translate('coverage'), "${_coveragePercent.toInt()}%", Icons.map_rounded, const Color(0xFFFB8C00)),
        _buildModernStatCard(context.translate('user_mgmt'), "$_residentsCount", Icons.people_alt_rounded, const Color(0xFF8E24AA)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(children: [
        Padding(padding: const EdgeInsets.all(24), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Row(children: [Container(width: 12, height: 12, decoration: const BoxDecoration(color: Color(0xFF00C853), shape: BoxShape.circle)), const SizedBox(width: 12), Text(context.translate('live_tracking'), style: const TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A), fontSize: 18))]),
          TextButton.icon(onPressed: () => setState(() => _selectedIndex = 1), icon: const Icon(Icons.open_in_new_rounded, size: 16), label: Text(context.translate('full_map'), style: const TextStyle(fontWeight: FontWeight.w900)), style: TextButton.styleFrom(foregroundColor: const Color(0xFF00BFA5), backgroundColor: const Color(0xFFE0F2F1), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
        ])),
  ...
  Widget _buildBottomNav() {
    return Container(
      ...
      child: BottomNavigationBar(
        ...
        items: [
          BottomNavigationBarItem(icon: const Icon(Icons.dashboard_rounded), label: context.translate('home')), BottomNavigationBarItem(icon: const Icon(Icons.map_rounded), label: context.translate('track')), BottomNavigationBarItem(icon: const Icon(Icons.analytics_rounded), label: context.translate('analytics')), BottomNavigationBarItem(icon: const Icon(Icons.chat_bubble_rounded), label: context.translate('complaints')), BottomNavigationBarItem(icon: const Icon(Icons.people_outline_rounded), label: context.translate('user_mgmt')), BottomNavigationBarItem(icon: const Icon(Icons.storage_rounded), label: context.translate('data_mgmt')), BottomNavigationBarItem(icon: const Icon(Icons.settings_rounded), label: context.translate('settings')),
        ],
