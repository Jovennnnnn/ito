...
class _ResidentDashboardState extends State<ResidentDashboard> {
        ...
        child: Column(children: [
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(context.translate('welcome_back'), style: const TextStyle(color: Colors.white70, fontSize: 14, fontWeight: FontWeight.w600)),
          ...
          Transform.translate(
            ...
            child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(children: [
              Expanded(child: _buildStatCard(context.translate('active_trucks'), "$_activeTrucks / $_totalTrucks", Icons.local_shipping_outlined, const Color(0xFF00BFA5), const Color(0xFFE8F5E9))),
              const SizedBox(width: 12),
              Expanded(child: _buildStatCard(context.translate('estimated_time'), _etaText, Icons.access_time_rounded, const Color(0xFFFFA000), const Color(0xFFFFF8E1))),
            ])),
          ),
          _buildSectionTitle(context.translate('real_time_overview')),
          Container(
            ...
            child: Column(children: [
              ListTile(title: Text(context.translate('live_tracking'), style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17)), subtitle: const Text("Real-time truck locations", style: TextStyle(fontSize: 12)), trailing: TextButton(onPressed: () => setState(() => _selectedIndex = 1), child: Text(context.translate('full_map')))),
          ...
          _buildSectionTitle(context.translate('quick_actions')),
          if (width > 700) Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Wrap(spacing: 12, runSpacing: 12, children: [
            _buildActionCard(context.translate('track_trucks'), "Real-time location", Icons.local_shipping_rounded, const Color(0xFF1E88E5), const Color(0xFFE3F2FD), () => setState(() => _selectedIndex = 1), width: (width - 52) / 2),
            _buildActionCard(context.translate('file_complaint'), "Report issues", Icons.feedback_outlined, const Color(0xFFFF1744), const Color(0xFFFFF0F2), () => Navigator.pushNamed(context, '/file_complaint'), width: (width - 52) / 2),
          ])) else Column(children: [
            _buildActionCard(context.translate('track_trucks'), "Real-time location", Icons.local_shipping_rounded, const Color(0xFF1E88E5), const Color(0xFFE3F2FD), () => setState(() => _selectedIndex = 1)),
            _buildActionCard(context.translate('file_complaint'), "Report issues", Icons.feedback_outlined, const Color(0xFFFF1744), const Color(0xFFFFF0F2), () => Navigator.pushNamed(context, '/file_complaint')),
          ]),
  ...
  Widget _buildBottomNav() {
    return BottomNavigationBar(
      ...
      items: [
        BottomNavigationBarItem(icon: const Icon(Icons.home_rounded), label: context.translate('home')),
        BottomNavigationBarItem(icon: const Icon(Icons.location_on_rounded), label: context.translate('track')),
        BottomNavigationBarItem(icon: const Icon(Icons.chat_bubble_rounded), label: context.translate('complaints')),
        BottomNavigationBarItem(icon: const Icon(Icons.settings_suggest_rounded), label: context.translate('settings')),
      ],
  ...
  void _showNotificationsModal(BuildContext context) {
    showDialog(context: context, builder: (context) => Dialog(
      ...
      child: Container(constraints: const BoxConstraints(maxWidth: 500), padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [const Icon(Icons.notifications_active_outlined, color: Color(0xFF00BFA5), size: 28), const SizedBox(width: 12), Expanded(child: Text(context.translate('notifications'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)))]),
        ...
        Flexible(child: StreamBuilder(stream: _database.ref('notifications').onValue, builder: (context, snapshot) {
          ...
          return Center(child: Text(context.translate('no_notifications')));
        })),
